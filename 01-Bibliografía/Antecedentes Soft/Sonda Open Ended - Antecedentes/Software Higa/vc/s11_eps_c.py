...
def fun(xs, f, s11_mod, s11_ph):
    ...
    s11_real = s11_mod*cos(s11_ph*np.pi/180)
    s11_img = s11_mod*sin(s11_ph*np.pi/180)
...
with open(file_in, 'rb') as csvfile:
    ...
    for f_str, s11_mod_str, s11_phase_str in reader:
        ...
        if mesg!='The solution converged.':        
            print i+1,mesg        
        i = i+1        
        er1.append((f, er1_real,er1_imag))
        
with open(file_out, 'wb') as csvfile:
    writer = csv.writer(csvfile, delimiter=';')
    i = 0
    for er1_med in er1:
        writer.writerow( [str(er1_med[0]).replace('.',',')] + [str(er1_med[1]).replace('.',',')] + [str(er1_med[2]).replace('.',',')])
        i = i+1